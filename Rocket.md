...
2. 标记-清除：先标记需要清除的对象，然后再清除。缺点1：如果标记的对象过多，那么效率会下降的很快，缺点2：清除之后会产生内存碎片；
3. 标记-复制：将内存划分为一块eden区域和两块survivor,大小比例是8：1：1，每次垃圾搜集的时候，将eden和一块survivor区内存活的对象复制到另一块survivor区，然后直接清除eden区和使用过的survivor区，这样保证了不会有太多的空间浪费同时解决了内存碎片的问题。
