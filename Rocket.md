...
## 职业规划
- 长期目标：5年内成为一名架构师
- 短期目标：还结合公司业务，深入学习理解公司的相关框架和技术，以技术积累为主。
## 最有成就感的事haishiyi
我最有成就感的事情是解决了一次服务网络问题
问题现象：有一家短信服务提供商接口偶尔报连接超时异常
初步调查：当时其他的接口都没有报错，初步判定是这家短信服务商有问题，我们将异常日志提交给他们之后，对方回答没有发现异常日志（当时其实不太相信，应为之前他们服务返回500，也不承认）；
进一步调查：到底是网络层（跨国）有问题，还是对方主机有问题，于是起了几个脚本循环ping，mtr去探测对方的域名，等到接口再次出现异常的时候，我们再去分析mtr的日志，这里需要注意的是有的节点把icmp协议限速了或者是ban了，要排除掉这种因素影响，发现有几个节点丢包比较严重，所以初步判断确实网络有问题
解决方案：使用阿里云专门的印尼网络隧道，开始的时候把30%的流量切到隧道，等到下次异常发生的时候，对比隧道的报错和正常路由的报错，发现隧道的请求都是正常的，所以基本判定是网络问题，于是把所有的流量切到隧道上去。同时吸取教训，把我们其他的网络服务域名也监控起来，方便下次出错的时候排查问题。
...
### 顺序消息原理
消息发送的时候，按顺序排好同步发送到同一个队列里面去（比如abc3条消息都携带订单号，都按照订单号取模，只要模相同，他们就会发送到一个队列里面）
消费的时候，用一个线程去消费同一个队列里面的消息即可

### 事务消息原理

1.事务消息发送及提交：

(1) 发送消息（half消息，写在一个专门的事务topic里面，对消费者不可见）。
(2) 服务端响应消息写入结果。
...
1. String字符串:字符串类型是 Redis 最基础的数据结构，首先键都是字符串类型，而且 其他几种数据结构都是在字符串类型基础上构建的，我们常使用的 set key value 命令就是字符串。常用在缓存、计数、共享Session、限速等。
   - 底层是动态字符串，对原生的char数组进行了封装，解决了原生c字符串（char数组）的以下几个问题
     - 维护了字符串长度变量（原生char数组需要遍历之后才能获取长度）
     - 杜绝缓冲区溢出（对value进行修改时，会先检查空间是否足够，不够的话会进行扩容，原生char数组比如在拼接的时候，容易出现溢出）
     - 采用空间预分配和惰性空间释放，来减少修改字符串带来的内存重分配次数
     - 可以保留任意格式的二进制数据（c字符串的字符必须符合某种编码）
2. Hash哈希:在Redis中，哈希类型是指键值本身又是一个键值对结构，哈希可以用来存放用户信息，比如实现购物车。
   - ziplist压缩列表,hashtable哈希表。只有当存储的数据量比较小的情况下，Redis 才使用压缩列表来实现字典类型
3. List列表（双向链表）:列表（list）类型是用来存储多个有序的字符串。可以做简单的消息队列的功能。
   - 可以认为quickList，是ziplist（变长数组）和linkedlist二者的结合；
...
Redis中setnx不支持设置过期时间，做分布式锁时要想避免某一客户端中断导致死锁，需设置lock过期时间，在高并发时 setnTNX成功，Ex与 expire 不能实现原子操作，如果要用，得在程序代码上显示的加锁。使用SET代替SETNX ，相当于SETNX+EXPIRE实现了原子性，不必担心SEXPIRE失败的问题。

### redisson基本获取锁的原理
锁数据结构：hash
先判断锁是否存在，如果不存在就调用hincrby，外层key是锁的name，内层key是线程id，再设置锁的超时时间，返回成功，启动watchdog定时检查客户端是否持有锁，不断延长锁的生存时间 ，如果锁存在，再判断hexist判断自己的线程id是否在hash里面，如果不在就获取锁失败，随即订阅锁释放的事件，如果等到了锁释放，那么会在最大等待时间内尝试再次获取锁
...
### mysql死锁案例
两个线程批量update导致死锁。因为mysql只有在具体update某一行数据的时候才会加锁，比如一个线程update了1这条数据，等待着update 2，但是另一个线程update了2等待着update 1，这就造成了死锁
解决办法：外面套了一层redis锁保证更新是串行的
...
- 分析器： 没有命中缓存的话，SQL 语句就会经过分析器，分析器说⽩了就是要先看你的 SQL语句要⼲嘛，再检查你的 SQL 语句语法是否正确。
...
* MVCC多版本并发控制是MySQL中基于乐观锁理论实现隔离级别的方式，用于读已提交和可重复读取隔离级别的实现。在MySQL中，会在表中每一条数据后面添加两个字段：最近修改该行数据的事务ID，指向该行（undolog表中）回滚段的指针。Read View判断行的可见性，创建一个新事务时，copy一份当前系统中的活跃事务列表。意思是，当前不应该被本事务看到的其他事务id列表。已提交读隔离级别下的事务在每次查询的开始都会生成一个独立的ReadView,而可重复读隔离级别则在第一次读的时候生成一个ReadView，之后的读都复用之前的ReadView。
  在开启事务之后，会保存当前开始但未提交的事务id集合，如果一条数据的事务id大于该集合的最大值，不可见，如果小于最小值，可见，最后如果在集合内，不可见，否则可见。
### binlog和redolog的区别
...
## JVM
### JMM
jmm的目的是定义程序中各种变量的访问规则，规定所有的变量都存储在主内存（堆），每条线程还会有自己的工作内存（栈），线程对变量的操作都必须在工作内存中进行，不可以直接读写主内存的数据，线程间工作内存是隔离的，线程间的变量值的传递均需要通过主内存来完成。

### 如何判断对象已死
- 引用计数法：原理简单，效率也高，但是也有很多情况需要处理，比如两个对象循环引用，会导致谈们的计数器都不为0从而无法回收
- 可达性分析算法：以gcroot根对象作为起点，向下搜索引用链，如果对象到gcroot没有任何引用链，那么就认为该对象不可达
  - gcroot：虚拟机栈中引用的对象，磊静态属性引用的对象，常量引用的对象
...
1. 标记-清除：先标记需要清除的对象，然后再清除。缺点1：如果标记的对象过多，那么效率会下降的很快，缺点2：清除之后会产生内存碎片；
2. 标记-复制：将内存划分为一块eden区域和两块survivor,大小比例是8：1：1，每次垃圾搜集的时候，将eden和一块survivor区内存活的对象复制到另一块survivor区，然后直接清除eden区和使用过的survivor区，这样保证了不会有太多的空间浪费同时解决了内存碎片的问题。
3. 标记-整理：相比较标记清除，需要让存活的对象向内存的一端移动，然后擦除边界以外的内存。
4. 分代收集算法：把Java堆分为新生代和老年代，然后根据各个年 代的特点采用最合适的收集算法。新生代中，对象的存活率比较低，所以选用复制算法，老年代中对象存活率高且没有额外空间对它进行分配担保，所以使用“标记-清除”或“标记-整理”算法进行回收。
...
- 保证变量对所有线程的可见性
- 禁止指令排序
通过字节码来分析，在对volatile修饰的变量赋值之后，后面会多一条lock指令, 该指令会将处理器缓存写入到内存，并且无效化其他处理器的缓存，所以保障了可见性，并且lock之后的指令不能在lock之前执行，也就是禁止指令排序
...
如果在这段期间它的值曾经被改成了B，后来又被改回为A，那CAS操作就会误认为它从来没有被改变过。Java并发包为了解决这个问题，提供了一个带有标记的原子引用类“AtomicStampedReference”，里面有一个类似版本号字段int类型字段stamp，它可以通过控制变量值的版本来保证CAS的正确性。
...
AQS主要有3个对象，一个是state变量（锁资源），两个队列，一个是等待队列，一个是条件队列。
基本流程如下：比如有两个线程a，b去抢写锁，他们都会通过cas尝试将state从0改成1，假如a成功了，a会作为等待队列的头节点，b会在a节点后面。如果a调用condition.await（），那么a会让出锁资源，进入条件队列，并唤醒b抢锁，b就会抢占到锁资源。如果b调用condition.signal（），那么会将a从条件队列中出来
进入到等待队列并尝试抢锁
### 如何指定多个线程的执行顺序
...
三级缓存详解：[没人比我更懂循环依赖！](https://juejin.cn/post/6860785780307492871)

### Spring如何解决循环依赖问题
场景：AB之间互相依赖
1. A在实例化之后，属性赋值之前，会将自己放入第三级缓存;
2. A属性赋值阶段触发实例化B，B在实例化之后，属性赋值之前，会将自己放入第三级缓存;
3. B发现自己依赖A，会从3级缓存中获取A的实例，并将A移除第三级缓存，放入第二级缓存。
4. B完成初始化之后，b实例放入一级缓存，并从二三级缓存移除中移除
5. 继续A实例的初始化，将B注入A，A在完成初始化后，A实例放入1级缓存，从其他缓存中移除
#### 为什么需要3级缓存？
在bean初始化的阶段，bean是可能被替换掉的，会导致互相拿到的对象不是同一个对象，所以引入了3级缓存，提早将需要替换的bean替换出来，在A初始化的时候，如果二级缓存存在，就直接使用
#### 思考：为什么多例的bean不支持循环依赖
尼玛多例的话，谁特么知道该注入哪个bean，卧槽
...
1. 尽量缩小锁的范围，合理设计索引
2. 减小锁冲突的几率，大事务拆解成小事务
3使用等值查询而不是范围查询查询数据，命中记录，避免间隙锁对并发的影响。
## 计算机网路
